...
// copyStatusMsg is sent to clear the copy status message after a delay
type copyStatusMsg struct{}

// copyResultMsg is sent when copy operation completes
...
type Model struct {
	...
	// Test/control flags
	disableAutoLoad bool // when true, Init will not trigger schema loading
}
...
func NewModel(width, height int) Model {
	...
	return Model{
		...
		exportViewport: viewport.New(width-4, height-6), // Account for title(1), borders(2), status bar(1), padding(2)
	}
}
...
func (m Model) Init() tea.Cmd {
	if m.disableAutoLoad {
		// Used by tests that inject schemas directly
		return nil
	}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
		...
		if km, ok := msg.(tea.KeyMsg); ok {
			...
			case "enter":
				...
				if entitySchema != nil {
					...
					m.exportViewport.SetContent(m.exportResult)
					m.exportViewport.GotoTop()
					m.stage = StageExportResult
					m.status.SetHelpText("j/k scroll • c copy • esc return")
					m.tree.Blur()
				}
				m.exportNamePrompt = false
				return m, nil
	...
	case tea.WindowSizeMsg:
		...
		// Update export viewport dimensions
		viewportWidth := m.width - 4
		availableHeight := m.height - 6 // title(1) + borders(2) + status bar(1) + padding(2)
		if viewportWidth < 10 {
			viewportWidth = 10
		}
		...
		m.exportViewport.Width = viewportWidth
		m.exportViewport.Height = availableHeight
	...
	case copyStatusMsg:
		...
		m.status.ClearCopyStatus()

	case copyResultMsg:
		if msg.success {
			m.status.SetCopyStatus("✓ Copied to clipboard!", "success")
		} else {
			m.status.SetCopyStatus("✗ Failed to copy to clipboard", "error")
		}
		// Set timer to clear the message after 3 seconds
		return m, tea.Tick(3*time.Second, func(time.Time) tea.Msg {
			return copyStatusMsg{}
		})
	...
	case tea.KeyMsg:
		...
		if m.stage == StageTreeView && m.focus == FocusTree {
			...
			case "pgup":
				...
				return m, nil
			case "esc":
				// If there is any selection, clear it first; otherwise, go back
				if len(m.tree.GetSelectedPaths()) > 0 {
					m.tree.ClearSelection()
					return m, nil
				}
		...
		// Export view navigation keys
		if m.stage == StageExportResult {
			var cmd tea.Cmd
			switch msg.String() {
			case "j", "down":
				m.exportViewport, cmd = m.exportViewport.Update(tea.KeyMsg{Type: tea.KeyDown})
				return m, cmd
			case "k", "up":
				m.exportViewport, cmd = m.exportViewport.Update(tea.KeyMsg{Type: tea.KeyUp})
				return m, cmd
			case "pgdown":
				m.exportViewport, cmd = m.exportViewport.Update(tea.KeyMsg{Type: tea.KeyPgDown})
				return m, cmd
			case "pgup":
				m.exportViewport, cmd = m.exportViewport.Update(tea.KeyMsg{Type: tea.KeyPgUp})
				return m, cmd
			}
		}
		switch msg.String() {
...
func (m *Model) updateLayout() {
	...
	if m.stage == StageTreeView {
		...
		if providersHeight < 8 {
			providersHeight = 8
		} // Need space for title + content
		if typesHeight < 6 {
			typesHeight = 6
		} // Need space for title + items
		if entitiesHeight < 10 {
			entitiesHeight = 10
		}
...
func (m *Model) handleEscape() tea.Cmd {
	...
	case StageExportResult:
		...
		m.focus = FocusTree
		m.tree.Focus()
		m.status.SetHelpText("") // Clear export help text
		m.exportResult = ""
	}
...
func (m *Model) handleCopy() tea.Cmd {
	return func() tea.Msg {
		if err := CopyToClipboard(m.exportResult); err != nil {
			return copyResultMsg{success: false, err: err}
		}
		return copyResultMsg{success: true, err: nil}
	}
}
...
func (m Model) renderExportView() string {
	...
	content := lipgloss.NewStyle().
		Border(lipgloss.RoundedBorder()).
		Padding(1).
		Render(m.exportViewport.View())

	// Add status bar at bottom (like other views)
	statusView := m.status.Render()

	// Layout: title + content + status bar (no hint text)
	return lipgloss.JoinVertical(lipgloss.Top, title, content, statusView)
}
