...
func GetInstalledProviders(dir string) ([]ProviderInfo, error) {
	providerSchema, err := terraform.FetchAllProviderSchemas(dir)
	if err != nil {
		return nil, err
	}
	
	var providers []ProviderInfo
	for name := range providerSchema.ProviderSchemas {
		...
		version := ""
		if providerSchema.ProviderVersions != nil {
			version = providerSchema.ProviderVersions[name]
		}
